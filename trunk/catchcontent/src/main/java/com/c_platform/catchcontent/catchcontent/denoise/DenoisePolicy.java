...
import com.c_platform.catchcontent.catchcontent.util.ForumUtil;
...
public class DenoisePolicy {
	...
	public Element filterJsAdvertise(String url, Element element, String model) {
		if (url == null || "".equalsIgnoreCase(url.trim()) || element == null)
			return element;
//		AdvertiseUrlAndXpath advertiseUrlAndXpath = new AdvertiseUrlAndXpath();
//		AdvertiseUrl au = advertiseUrlAndXpath.getAdvertiseUrlByUrl(url.trim());
//		if (au == null) {
//			int tag = 0;
//			Iterator auList = advertiseUrlAndXpath.getAdvertiseUrlByType()
//					.iterator();
//			while (auList.hasNext()) {
//				au = (AdvertiseUrl) auList.next();
//				Pattern p = Pattern.compile(au.getUrl(),
...
//			}
//		}
//		if ("0".equalsIgnoreCase(au.getIsFilter()))
//			return element;
//		List<AdvertiseXpath> adsXpathList = advertiseUrlAndXpath
//				.getAdvertiseXpathById(au.getAuid());
...
//		ComparatorAdvertiseXpath comp = new ComparatorAdvertiseXpath();
//		Collections.sort(adsXpathList, comp);

//		ArrayList<AdvertiseXpath> tmpAdsXpathList = new ArrayList<AdvertiseXpath>();
//		for (AdvertiseXpath ax : adsXpathList) {
//			if (ax.getModel() == null)
//				continue;
//			if ("1".equalsIgnoreCase(ax.getIsUsing().trim())
//					&& ax.getModel().indexOf(model) >= 0) {
//				tmpAdsXpathList.add(ax);
//			}
//		}
		// 如果是RSS模式，没有相关的xpath，则使用云加速模式的xpath
//		if (model.equalsIgnoreCase("2") && tmpAdsXpathList.size() == 0) {
//			for (AdvertiseXpath ax : adsXpathList) {
//				if (ax.getModel() == null)
//					continue;
//				if ("1".equalsIgnoreCase(ax.getIsUsing().trim())
//						&& ax.getModel().indexOf("1") >= 0) {
//					tmpAdsXpathList.add(ax);
//				}
//			}
//		}
//		for (AdvertiseXpath ax : tmpAdsXpathList) {
//			try {
//				List<Element> elementList = element.selectNodes(ax.getXpath());
//				for (Element e : elementList) {
//					Element tmp = e;
//					boolean tag = true;
//					while (tag) {
//						String s = tmp.getQName().getName();
//						if (s.equalsIgnoreCase(HEAD)) {
//							if (ax.getHandleModel().trim()
//									.equalsIgnoreCase("0")) {
//								headFilter.add(e);
//							} else {
//								headReserve.add(e);
