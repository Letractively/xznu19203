...
public class BilldbHelper {
	...
	public void createTable_acctitem() {
		try {
			db.execSQL("CREATE TABLE acctitem (" + "ID INTER PRIMARY KEY,"
					+ "PID INTEGER," + "NAME TEXT," + "TYPE INTEGER" + ");");
	...
	public void createTable_bills() {
		try {
			db.execSQL("CREATE TABLE bills (" + "ID INTEGER PRIMARY KEY,"
					+ "FEE INTEGER," + "USERID TEXT," + "SDATE TEXT,"
	...
	public void createTable_colaconfig() {
		try {
			db.execSQL("CREATE TABLE colaconfig (" + "ID INTEGER PRIMARY KEY,"
					+ "NAME TEXT" + ");");
	...
	public void FirstStart() {
		...
		try {
			...
			Cursor c = db.query("sqlite_master", col, "name='colaconfig'",
					null, null, null, null);
			int n = c.getCount();
			if (c.getCount() == 0) {
