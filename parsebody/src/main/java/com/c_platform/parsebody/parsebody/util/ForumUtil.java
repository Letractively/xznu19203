...
public class ForumUtil {
	...
	public static void removeHeightWidth(Element element, WebContext ctx)
		...
		for (int i = element.attributeCount() - 1; i >= 0; i--) {
			if (element.attribute(i).getName().trim().equalsIgnoreCase("width")
					|| element.attribute(i).getName().trim()
							.equalsIgnoreCase("height")
					|| element.attribute(i).getName().trim()
							.equalsIgnoreCase("cols")) {
				element.remove(element.attribute(i));
			} else if (element.attribute(i).getName().trim()
	...
	/*
	 * 创建登录注册元素节点 主要针对的网站有：安智论坛
	 */
	public static Element createUMenu(URL baseUrl) throws Exception {
		Element result = null;
		String registerUrl = UrlUtil.guessURL(baseUrl, "register.php")
				.toString();
		String loginUrl = UrlUtil.guessURL(baseUrl, "logging.php?action=login")
				.toString();
		String text = "<DIV depth=\"5\" id=\"umenu\"><A href=\"" + registerUrl
				+ "\">注册  </A><A href=\"" + loginUrl + "\">  登录</A></DIV>";
	...
	public static Element createMenu(URL baseUrl) throws Exception {
		Element result = null;
		String registerUrl = UrlUtil.guessURL(baseUrl, "register.php")
				.toString();
		String loginUrl = UrlUtil.guessURL(baseUrl, "logging.php").toString();
		String text = "<DIV depth=\"5\" id=\"umenu\"><A href=\"" + registerUrl
				+ "\">注册  </A><A href=\"" + loginUrl + "\">  登录</A></DIV>";
		Document document = DocumentHelper.parseText(text);
		result = document.getRootElement();
